                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {cartItems.map(({ _id, item, quantity }) => (
                                <div
                                    key={_id}
                                    className="group bg-amber-900/20 p-4 rounded-2xl border-4 border-dashed border-amber-800 backdrop-blur-sm flex flex-col items-center gap-4 transition-all duration-300 hover:border-solid hover:shadow-xl hover:shadow-amber-900/10 hover:-translate-y-1 animate-fade-in transform"
                                >
                                    ...
                                    <div className='w-full text-center'>
                                        <h3 className='text-Frexl font-dancingscript text-amber-100'>
                                            {item.name}
                                        </h3>
                                        <p className='text-amber-100/50 font-cinzel mt-1'>₹{Number(item.price).toFixed(2)}</p>
                                    </div>
                                    ...
                                    <div className='flex items-center gap-3'>
                                        <button onClick={() => updateQuantity(_id, Math.max(1, quantity - 1))}
                                            ...
                                            <FiMinus className='w-8 text-amber-100 ' />
                                        </button>
                                        ...
                                        <button onClick={() => updateQuantity(_id, quantity + 1)}
                                            className='w-8 h-8 rounded-full bg-amber-900/40 flex items-center justify-center hover:bg-amber-800/50 transition-all duration-200 active:scale-95'>
                                            <FiPlus className='w-8 text-amber-100 ' />
                                        </button>
                                    </div>
