  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (res.status === 200 && res.data.success && res.data.token) {
        ...
        localStorage.setItem('loginData', JSON.stringify({ loggedIn: true, email: formData.email, rememberMe: formData.rememberMe }));
    ...
    <div className='space-y-6 relative '>
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        <div className='relative'>
          ...
          <input type="email" name='email' placeholder='email' value={formData.email} onChange={handleChange} className={`${inputBase} pl-10 pr-4 py-3`} />
        </div>
        <div className='relative'>
          <FaLock className={iconClass} />
          <input type={showPassword ? 'text' : 'password'} name='password' placeholder='Password' value={formData.password} onChange={handleChange} className={`${inputBase} pl-10 pr-10 py-3`} />
        ...
        <div className='flex items-center'>
          <label className='flex items-center'>
            <input type="checkbox" name='rememberMe' checked={formData.rememberMe} onChange={handleChange} className='form-checkbox h-5 w-5 text-amber-600 bg-[#2D1B0E] border-amber-400 rounded
            focus:ring-amber-600' />
            <span className='ml-2 text-amber-100'>Remember Me</span>
          </label>
        </div>
        <div className='text-right -mt-3'>
          <Link to="/forgot-password" onClick={onclose} className='text-amber-400 hover:text-amber-600 text-sm transition-colors'>
            Forgot Password?
          </Link>
        </div>
